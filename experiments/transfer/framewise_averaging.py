...
from evaluation import EEGNet, get_fold, add_kernel_dim, onehot, test_val_rest_split, test_model, test_framewise_avg
...
chans, samples = _pilotX.shape[1], _pilotX.shape[2] - AVG_FRAMES
...
for i in range(0, FOLDS):
  ...
  """
  framewise averaging stuff
  """
  # slice of last AVG_FRAMES samples from the training and validation data
  comp_trainX, comp_valX = comp_trainX[:,:,:,:-AVG_FRAMES], comp_valX[:,:,:,:-AVG_FRAMES]
  print(comp_trainX.shape, comp_trainY.shape)
  print(comp_valX.shape, comp_valY.shape)
  ...
  comp_eval = test_framewise_avg(model, comp_testX, comp_testY, AVG_FRAMES)
  ...
  for i, (train_index, test_index) in enumerate(skf.split(_pilotX, _pilotY)):
    ...
    framewise averaging stuff
    """
    # slice of last AVG_FRAMES samples from the transfer learning training and validation data
    pilot_trainX, pilot_valX = pilot_trainX[:,:,:,:-AVG_FRAMES], pilot_valX[:,:,:,:-AVG_FRAMES]

    # transter learn on pilot data
    train(model, {"x": pilot_trainX, "y": pilot_trainY}, {"x": pilot_valX, "y": pilot_valY}, epochs=TRANSFER_EPOCHS)

    # test pilot data
    pilot_eval = test_framewise_avg(model, pilot_testX, pilot_testY, AVG_FRAMES)
